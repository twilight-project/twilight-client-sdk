...
use crate::transfer::{TransferTxMultipleAccountsWallet, TransferTxWallet};
    ...
    while initial_balance >= 800 {
        match create_db_accounts(sender_address.clone(), initial_balance, sk) {
            ...
            Err(e) => {
                ...
                if e == "Verification Error: sender account verification failed".to_string() {
                    if balance_flag == 0 {
                        initial_balance += 800;
                        println!(
                            "updated Balance due to block missing: {:?}",
                            initial_balance
                        );
                        balance_flag = 1;
                        ...
                        initial_balance -= 800;
                        println!(
                            "updated Balance -ve due to block missing: {:?}",
                            initial_balance
                        );
                    } else {
                        balance_flag = 0;
...
pub fn update_settled_accounts_in_db_service(sk: RistrettoSecretKey) {
    ...
    loop {
        ...
        for account in settled_accounts {
            ...
            let input = match crate::chain::get_transaction_coin_input_from_address_fast(
                pk_address.clone(),
            ) {
                Ok(inp) => inp,
                Err(e) => {
                    println!(
                        "Error in Fetching Utxo: {:?} , Adddess:{:?}",
                        e,
                        pk_address.clone()
                    );
                    continue;
            ...
            match convert_encrypted_trading_account_to_scalar_based_account_db(
                pk_address.clone(),
                sk,
                value,
            ) {
...
pub fn test_tx_commit_rpc(sk: RistrettoSecretKey) {
    ...
    for i in 0..900 {
        ...
        let (_, upd_sender, _) = self::create_boadcast_transfer_tx_single_account(
            coin_address.clone(),
            sk,
            updated_sender_amount,
            10,
        );
...
fn single_to_multiple_account_transfer(
    ...
    sender_address: String,
    balance: u64,
) -> Result<TransferTxMultipleAccountsWallet, String> {
    let mut rng = rand::thread_rng();

    //get coin input from output
    let bob_input_1 = crate::chain::get_transaction_coin_input_from_address_fast(sender_address)?;
...
pub fn single_source_single_reciever_transfer(
    ...
    //get coin input from output
    let input = match crate::chain::get_transaction_coin_input_from_address_fast(coin_address) {
        Ok(input) => input,
    ...
    match single_to_multiple_account_transfer(sk, address.clone(), initial_balance) {
        Ok(tx_wallet) => {
            ...
            let _response: String =
                match crate::chain::tx_commit_broadcast_transaction(tx_wallet.get_tx().unwrap()) {
            ...
            for i in 0..10 {
                ...
                if utxo_id.is_ok() {
                    ...
                    for i in 0..7 {
                        let scalar_str = encrypt_scalar[i].clone();
...
pub fn create_boadcast_transfer_tx_single_account(
    ...
    (
        tx_wallet.get_encrypt_scalar().unwrap().clone(),
        updated_sender_balance,
        reciever_address.clone(),
    )
}

pub fn convert_encrypted_trading_account_to_scalar_based_account_db(
    coin_address: String,
    sk: RistrettoSecretKey,
    value: u64,
) -> Result<(), String> {
    ...
    // send the tx to chain
    let _response = crate::chain::tx_commit_broadcast_transaction(tx_wallet.get_tx().unwrap())?;
    ...
    for i in 0..5 {
        ...
        if utxo_id.is_ok() {
            ...
            let scalar_str = crate::util::scalar_to_hex(tx_wallet.get_encrypt_scalar().unwrap());
            let balance = value;
