...
use zkos_client_wallet::agent::auto_accounts::get_account_balance;
lazy_static! {
    ...
    //     std::env::var("ACCOUNT_1_BALANCE").expect("missing environment variable ACCOUNT_1_BALANCE");
    // pub static ref ACCOUNT_2_BALANCE: String =
    //     std::env::var("ACCOUNT_2_BALANCE").expect("missing environment variable ACCOUNT_2_BALANCE");
}
...
fn create_db_accounts_from_main_trading_account1(sk: RistrettoSecretKey) {
    ...
    let path = "ACCOUNT_1_BALANCE.txt";
    let initial_amount: u64 = get_account_balance(path);
...
fn create_db_accounts_from_main_trading_account2(sk: RistrettoSecretKey) {
    ...
    let path = "ACCOUNT_2_BALANCE.txt";
    let initial_amount: u64 = get_account_balance(path);
    zkos_client_wallet::agent::auto_accounts::load_accounts_to_db_from_main_account(
        sk,
        client_address.to_string(),
