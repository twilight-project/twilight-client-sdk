...
pub fn broadcast_contract_deploy_transaction(
    ...
    Ok("Feature disabled in WASM build for browser compatibility".to_string())
}
...
pub fn create_contract_deploy_transaction(
    ...
    let coin_input = match serde_json::from_str(&coin_input) {
        Ok(coin) => coin,
        Err(e) => return Err(format!("Error at deserializing coin input :{:?}", e).into()),
    };
