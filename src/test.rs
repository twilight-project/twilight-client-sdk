...
//     use crate::test::{RELAYER_SEED, TEST_SEED};
...
//     use zkvm::{zkos_types::Utxo, Input, Output};

//     #[test]
//     fn test_keypair_verification() {
//         let sk = <RistrettoSecretKey as quisquislib::keys::SecretKey>::from_bytes(
//             RELAYER_SEED.as_bytes(),
//         );
...
//         // // get output
//         let coin_output =
//             crate::chain::get_coin_output_by_utxo_id_hex(utxo_id_vec[0].clone()).unwrap();
...
//         let bytes: [u8; 32] = [
//             173, 247, 181, 254, 22, 93, 68, 194, 234, 210, 165, 132, 45, 134, 236, 90, 255, 120,
//             236, 55, 33, 184, 126, 154, 23, 105, 254, 168, 81, 113, 10, 10,
//         ];
//         let encryption_commitment_scalar = Scalar::from_canonical_bytes(bytes).unwrap();

//         // recreate encryption with scalar and balance
//         let encrypt_new = ElGamalCommitment::generate_commitment(
//             &pk,
...
//             // get Account from Output
//             let utxo_id_vec =
//                 crate::chain::get_coin_utxo_by_address_hex(pk_address.to_string()).unwrap();
//             let coin_output =
//                 crate::chain::get_coin_output_by_utxo_id_hex(utxo_id_vec[0].clone()).unwrap();
//             let account: Account = Output::to_quisquis_account(&coin_output).unwrap();
//             let (pk_old, enc_old) = account.get_account();
//             let encrypt_new =
//                 ElGamalCommitment::generate_commitment(&pk_old, scalar, Scalar::from(balance));
//             // compare enc
//             assert_eq!(encrypt_new, enc_old);
//         }
...
//         let response =
//             crate::chain::tx_commit_broadcast_transaction(tx_wallet.get_tx().unwrap()).unwrap();
//         println!("response {:?}", response);

//         // sleep for 2 seconds to let the utxo set update
//         std::thread::sleep(Duration::from_secs(2));
//         let commitment_scalar = tx_wallet.get_encrypt_scalar().unwrap().clone();
...
//         let bytes = [
//             77, 248, 36, 68, 30, 218, 138, 123, 106, 210, 26, 141, 28, 98, 78, 29, 101, 134, 227,
//             150, 204, 70, 107, 172, 83, 59, 127, 181, 38, 52, 56, 8,
//         ];
...
//         let (fay_account, fay_comm_rscalar) = Account::generate_account(
//             PublicKey::update_public_key(&alice_key, Scalar::random(&mut rng)),
//         );
//         let (jay_account, jay_comm_rscalar) = Account::generate_account(
//             PublicKey::update_public_key(&alice_key, Scalar::random(&mut rng)),
//         );
...
//         ];
//         let tx_wallet =
//             crate::transfer::create_private_transfer_transaction_single_source_multiple_recievers(
//                 tx_vector,
//                 bob_input_1,
//                 bob_sk_account_1,
...
//         let tx = tx_wallet.get_tx().unwrap();
//         // verify the tx
...
//         let balance = 1000;
//         let account = crate::db_ops::create_account(
//             &mut conn,
//             pk_address,
//             Some(scalar_str),
//             is_on_chain,
...
//         let balance = 0;
//         let account =
//             crate::db_ops::create_account(&mut conn, pk_address, scalar_str, is_on_chain, balance)
//                 .unwrap();
//         println!("account {:?}", account);
//         // get account id
//         let id = account.id;
...
//         dotenv::dotenv().expect("Failed loading dotenv");
//         let mut conn = crate::db_ops::establish_connection();
//         let accounts = crate::db_ops::get_accounts_with_null_scalar_str(&mut conn, 20).unwrap();
...
//             let is_on_chain = false;
//             let balance = 0;
//             let _account = crate::db_ops::create_account(
//                 &mut conn,
//                 pk_address,
//                 scalar_str,
//                 is_on_chain,
...
//         let account_subset =
//             crate::db_ops::get_accounts_with_not_null_scalar_str(&mut conn, 15).unwrap();
